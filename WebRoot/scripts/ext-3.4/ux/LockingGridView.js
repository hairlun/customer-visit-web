...
Ext.ux.grid.LockingGridView = Ext.extend(Ext.grid.GridView, {
    lockText : '锁定当前列',
    unlockText : '解锁当前列',
    lockBeforeText : '锁定左侧列',
    unlockBeforeText: '解锁左侧列',
    rowBorderWidth : 1,
    lockedBorderWidth : 1,
    ...
    doRender : function(cs, rs, ds, startRow, colCount, stripe){
        var ts = this.templates, ct = ts.cell, rt = ts.row, last = colCount-1,
            ...
            buf = [], lbuf = [], cb, lcb, c, p = {}, rp = {}, r, cm = this.cm;
        ...
        if(g.enableHdMenu !== false){
            ...
            if(this.grid.enableColLock !== false){
                this.hmenu.add('-',
                    ...
                    {itemId: 'lockBefore', text: this.lockBeforeText, cls: 'xg-hmenu-lock'}
                );
            }
    ...
    handleHdMenuClick : function(item){
    	if (!this.aColumns) {
    		...
    		for (var i = 0; i < this.cm.getColumnCount(); i++) {
    			this.aColumns.push(this.cm.getColumnHeader(i));
    		}
    	}
        ...
        switch(text){
            ...
            break;
            case this.unlockText:
            	 var oldIndex = this.getOldIndex(cm.getColumnHeader(index));
            	 if (oldIndex < llen - 1) {
            		 cm.setLocked(index, false, true);
                     cm.moveColumn(index, llen - 1);
                     this.grid.fireEvent('columnmove', index, llen);
                 } else {
                	 cm.moveColumn(index, oldIndex);
                     cm.setLocked(index, false);
                 }
            break;
            case this.lockBeforeText:
            	this.lockBeforeColumns(index);
            break;
            case this.unlockBeforeText:
                this.unlockBeforeColumns(index);
            break;
            default:
    ...
    getOldIndex : function(colName){
    	for (var i = 0; i < this.aColumns.length; i++) {
    		if (this.aColumns[i] == colName) {
    			return i;
    		}
    	}
    },
    ...
    handleHdDown : function(e, t){
        ...
        if(this.grid.enableColLock !== false){
            if (Ext.fly(t).hasClass('x-grid3-hd-btn')) {
                ...
                if (cm.isLocked(index)) {
                	ms.get('lock').setText(this.unlockText);
                	ms.get('lock').removeClass("xg-hmenu-lock");
                	ms.get('lock').addClass("xg-hmenu-unlock");
                } else {
                	ms.get('lock').setText(this.lockText);
                	ms.get('lock').removeClass("xg-hmenu-unlock");
                	ms.get('lock').addClass("xg-hmenu-lock");
                }
                
                if (cm.isLocked(index)) {
                	ms.get('lockBefore').setText(this.unlockBeforeText);
                	ms.get('lockBefore').removeClass("xg-hmenu-lock");
                	ms.get('lockBefore').addClass("xg-hmenu-unlock");
                } else {
                	ms.get('lockBefore').setText(this.lockBeforeText);
                	ms.get('lockBefore').removeClass("xg-hmenu-unlock");
                	ms.get('lockBefore').addClass("xg-hmenu-lock");
                }   
            }
        }
    },
    ...
    lockBeforeColumns : function(idx){
    	for (var i = 0; i <= idx; i++) {
    		var cm = this.cm;
    		if (cm.getColumnHeader(i) == "" || cm.isLocked(i)) {
    			continue;
    		}
    		var index = i;
    ...
    unlockBeforeColumns : function(idx){
    	for (var i = idx; i >=0; i--) {
    		var cm = this.cm;
...
Ext.ux.grid.LockingColumnModel = Ext.extend(Ext.grid.ColumnModel, {
    ...
    isLocked : function(colIndex){
    	if (this.config[colIndex].columnLocked) {
