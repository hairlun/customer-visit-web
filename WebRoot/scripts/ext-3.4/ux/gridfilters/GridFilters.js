...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    createMenu : function () {
        ...
        if (this.showMenu && hmenu) {
            ...
            this.filterSpMenu.on({
				scope : this,
				checkchange: this.onSpCheckChange
			});
...
*/
            ...
            this.filterSpMenu.setChecked(filter.activeFlag, false);
            // disable the menu if filter.disabled explicitly set to true
            this.menu.setDisabled(filter.disabled === true);
            this.filterSpMenu.setDisabled(filter.disabled === true);
            if(filter.type=='boolean'|| filter.type=='numeric'){
            	 this.filterSpMenu.setVisible(false);
            }else{
            	this.filterSpMenu.setVisible(true);
            }
        }
    ...
    /** @private */
    onSpCheckChange : function (item, value) {
        this.getMenuFilter().setFlag(value);
    },

    /** @private */
    onBeforeCheck : function (check, value) {
//    	if(arguments.length==1){
//    		var value = Math.abs(this.flag-1);
//    		var boll = Boolean(value);
//    		this.flag = value;
//    		return boll;
//    	}
    ...
    updateColumnHeadings : function () {
        ...
        if (view.mainHd) {
            for (i = 0, len = view.cm.config.length; i < len; i++) {
                ...
                Ext.fly(view.getHeaderCell(i))[filter && (filter.active || filter.activeFlag) ? 'addClass' : 'removeClass'](this.filterCls);
            }
        }
    },
    ...
    getRecordFilter : function () {
        var f = [], len, i;
        this.filters.each(function (filter) {
            if (filter.active || filter.activeFlag) {
                f.push(filter);
            }
        });
        ...
        return function (record) {
            for (i = 0; i < len; i++) {
                if (f[i].active==true && !f[i].validateRecord(record)) {
                    return false;
                }else if (f[i].activeFlag==true && !(record.get(f[i].dataIndex)=='' || record.get(f[i].dataIndex)==null) ) {
                    return false;
                }
            }
            return true;
...
Ext.ux.grid.GridFilters.prototype.menuFilterText = '过滤';
