		...
		if (this.viewport) {
			...
			this.store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'interactiveItem.do?action=queryAll'
				}),
...
//				},
				...
				tbar : [{
					...
					width : 45,
					xtype : 'button',
					handler : this.newInteractiveItem,
					scope : this
				}, '-', {
					text : '编辑',
					iconCls : 'edit',
					...
					width : 45,
					xtype : 'button',
					handler : this.editInteractiveItem,
					scope : this
				}, '-', {
					text : '删除',
					iconCls : 'del',
					width : 45,
					xtype : 'button',
					handler : this.delInteractiveItem,
					scope : this
				}],
				store : this.store,
				sm : sm,
...
//					}]
				...
				buttons : [{
					...
					handler : function() {
						...
						Ext.haode.ajax({
							url : 'interactiveItem.do?action=addInteractiveItem',
							params : {
//								cids : cids,
...
//					}]
				...
				buttons : [{
					...
					handler : function() {
						...
						Ext.MessageBox.wait('正在操作','请稍后...');
						Ext.haode.ajax({
							url : 'interactiveItem.do?action=editInteractiveItem',
							params : {
								id : records[0].get('id'),
...
//		});
	...
	delInteractiveItem : function() {
		...
		Ext.haode.ajax({
			url : 'interactiveItem.do?action=delInteractiveItems',
			params : {
				ids : ids
			},
			success : function(json, opts) {
