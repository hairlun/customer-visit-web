		...
		if (this.viewport) {
			...
			this.store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'department.do?action=queryAll'
				}),
...
//					}]
				...
				buttons : [{
					...
					handler : function() {
						...
						Ext.haode.ajax({
							url : 'department.do?action=addDepartment',
							params : {
//								cids : cids,
...
//					}]
				...
				buttons : [{
					...
					handler : function() {
						...
						Ext.MessageBox.wait('正在操作','请稍后...');
						Ext.haode.ajax({
							url : 'department.do?action=editDepartment',
							params : {
								id : records[0].get('id'),
...
//		});
	...
	delDepartment : function() {
		...
		Ext.haode.ajax({
			url : 'department.do?action=delDepartments',
			params : {
				ids : ids
			},
			success : function(json, opts) {
