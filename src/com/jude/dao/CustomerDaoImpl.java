...
public class CustomerDaoImpl implements CustomerDao {
	...
	public PagingSet<Customer> getCustomers(int start, int pageSize, int groupId,
		...
		String sql = "select c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea, cgr.group_id from (customer c left join customer_manager m on c.manager_id = m.id) left join customer_group_relation cgr on c.id = cgr.customer_id where cgr.group_id = " + groupId;
		if (sort != null && !sort.equals("") && dir != null && !dir.equals("")) {
			sql += " order by " + sort + " " + dir;
		}
		PagingHelper h = new PagingHelper(this.jdbcTemplate);
	...
	public Customer getCustomer(int id) {
		List<Customer> customers = this.jdbcTemplate
				.query("select c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea from customer c left join customer_manager m on c.manager_id = m.id where c.id = "
	...
	public Customer getCustomerByNumber(String number) {
		String sql = "select c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea from customer c left join customer_manager m on c.manager_id = m.id where c.number = '"
				+ number + "'";
		PagingHelper h = new PagingHelper(this.jdbcTemplate);
	...
	public List<Customer> getCustomersByManagerIds(String ids) {
		String sql = "select c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea from customer c left join customer_manager m on c.manager_id = m.id where c.manager_id in ("
				+ ids + ")";
		return this.jdbcTemplate.query(sql, new CustomerRowMapper());
	}

	public List<Customer> getCustomersByGroupIds(String ids) {
		String sql = "select distinct c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea from (customer c left join customer_manager m on c.manager_id = m.id) left join customer_group_relation cgr on c.id = cgr.customer_id where cgr.group_id in ("
				+ ids + ")";
		return this.jdbcTemplate.query(sql, new CustomerRowMapper());
	}

	public List<Customer> getCustomersByIds(String ids) {
		String sql = "select c.id, c.number, c.name, c.sell_number, c.store_name, c.level, c.phone_number, c.backup_number, c.address, c.manager_id, c.order_type, c.gps, c.last_visit_time, m.name as mname, m.username as musername, m.password as mpassword, m.department as mdepartment, m.area as marea from customer c left join customer_manager m on c.manager_id = m.id where c.id in ("
				+ ids + ")";
		return this.jdbcTemplate.query(sql, new CustomerRowMapper());
	}
		...
		public Customer mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			try {
				...
				if ((musername != null) && (musername.length() > 0)) {
					...
					customerManager.setDepartment(rs.getString("mdepartment"));
					customerManager.setArea(rs.getString("marea"));
