...
//			throw x.syntaxError("Expected a ',' or ']'");
	...
	public JSONArray(Collection collection, PropertyFilter filter) {
		this.myArrayList = new ArrayList();
		if (collection != null) {
			Iterator iter = collection.iterator();
			while (iter.hasNext())
				this.myArrayList.add(JSONObject.wrap(iter.next(), filter));
		}
	}
	...
	public JSONArray(Object array, PropertyFilter filter) throws JSONException {
		if (array.getClass().isArray()) {
			int length = Array.getLength(array);
			for (int i = 0; i < length; ++i)
				put(JSONObject.wrap(Array.get(array, i), filter));
		} else {
